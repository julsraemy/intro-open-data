...
- **Linked Data and Storage:**  
  It stores DCAT metadata as RDF in a triplestore (with support for storage backends like Virtuoso and Fuseki) and utilises URI harmonisation to create a consistent knowledge graph. The platform also integrates external vocabularies and ontologies to enhance linked data capabilities.
...
- **Quality Assurance:**  
  Piveau periodically generates quality assessments based on SHACL validations and the FAIR principles. These assessments are stored alongside the metadata using the Data Quality Vocabulary (DQV), with reports available in various formats.
...
EntryScape is an information management platform designed to handle complex datasets through semantic integration and linked data principles. It facilitates the organisation and enrichment of information from varied sources [@ebnerInformationModelManaging2014]. Further information is available on the [EntryScape website](https://entryscape.com/) and via its repository on [Bitbucket](https://bitbucket.org/metasolutions/).

### uData

[uData](https://udata.readthedocs.io/) is an open data management platform developed by the Open Data Team. It aims to facilitate the publication and management of data in a user-friendly manner, making it easier for both data providers and users to work with datasets. The source code is available on their [GitHub repository](https://github.com/opendatateam/udata).
