---
title: "Syllabus"
authors: 
  - name: Julien A. Raemy
    orcid: 0000-0002-4711-5759
    email: julien.raemy@hesge.ch
    affiliations:
      - docuteam SA
      - University of Bern
date: "2024-12-12"
date-modified: last-modified
license: "CC BY"
---

## Learning Objectives 

This course introduces the foundational principles of Open Data, equipping undergraduate students in Information Science with the knowledge and skills to find, analyse, and effectively reuse open datasets. Through a combination of lectures, practical activities, and assignments, students will explore the benefits and challenges of Open Data and develop a deeper understanding of its potential applications and implications. It is designed with the following objectives:

1. To gain an understanding of Open Data, its essential aspects, and the principles of opening data;
2. to learn how to find, analyse, and reuse open datasets;
3. to learn the processes involved in preparing and publishing open datasets.

These objectives provide the foundation for the course and guide the learning outcomes.

## Content 

The topics covered in this course include:

- Background on open data;
- Benefits and risks of opening data;
- Models related to using and publishing open data.

## Methods

The course employs the following methods to support learning:

- Course presentations;
- Exploration of online services;
- Hands-on exercises;
- Engagement with scholarly literature.

These methods are intended to foster both theoretical understanding and practical application of the course material.

## Course Outline (2025-2026)

An exam (_contrôle continu_) will be held at the end of the fourth and final session. It will consist of multiple-choice and open-ended questions, will last a maximum of one hour, and will be conducted through Cyberlearn. All course notes and access to the internet are permitted. However, the use of Large Language Models (LLMs) and communication tools is prohibited.

| Date 	| Content 	|
|------------	|----------------------------	|
| 18.02.2026 	| Course Overview<br/>[Characteristics of Open Data](sections/characteristics.qmd)<br/>[Exercise 1: OA Deep Dive](exercises/ex-01.qmd)<br/>[Associated Movements](sections/movements.qmd)<br/>[Associated Principles](sections/principles.qmd)<br/>[Exercise 2: Movements and Principles](exercises/ex-02.qmd)<br/>[Exercise 3: Up to date with Linked Data](exercises/ex-03.qmd)	| 
| 25.02.2026 	| [Exercise 3: Up to date with Linked Data (Follow-up)](exercises/ex-03.qmd)<br/>[Open Data Platforms and Organisations](sections/platforms.qmd)<br/>[Exercise 4: Discovering ORD Platforms](exercises/ex-04.qmd)<br/>[Exercise 5: The Reuser's Perspective (OGD)](exercises/ex-05.qmd)<br/>[Exercise 6: Reading Assignment](exercises/ex-06.qmd)                  	|
| 04.03.2026 	| [Exercise 6: Reading Assignment (Follow-up)](exercises/ex-06.qmd)<br/>[Assessment, Data Quality, and Best Practices](sections/assessment.qmd)<br/>[Techniques, Software, and Tools](sections/techniques.qmd)<br/>[Exercise 7: Open Refine](exercises/ex-07.qmd)<br/>[Exercise 8: IIIF & ML](exercises/ex-08.qmd)                   	|
| 11.03.2026 	| [Exercise 8: IIIF & ML (Follow-up)](exercises/ex-08.qmd)<br/>[Showcases](sections/showcases.qmd)<br/>Overall Recap<br/>Examination (multiple-choice and open-ended)              	| 

::: {.callout-warning}
## Missed Examination Policy
If a student is absent during the last session and misses the examination for legitimate reasons, a substitute assignment will be organised by the tutor.
:::


