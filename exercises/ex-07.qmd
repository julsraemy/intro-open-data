...
6. **Explore with Facets**: 
   - Click the dropdown arrow in the `Categories` column header and select **Facet > Text facet**. 
   - A list of unique values appears on the left. Click on "Spacesuits" to filter the view, then click **Reset** to show all data again. 
   - *Goal:* Understand how facets allow you to filter data and spot inconsistencies without deleting anything.
...
- **For Step 6**: Facets are the most powerful tool in OpenRefine. They allow you to filter data and see errors (like typos or inconsistent capitalization) instantly.
...
13. **Reconcile with Biblissima**: 
    ...
    - Select the **Biblissima** service and choose the type **Person** (or *Autorité* depending on the endpoint response).
    - Click **Start Reconciling**.
    - Review the matches: 
        - **Green**: High confidence match.
        - **Yellow**: Multiple candidates (click the cell to choose the correct one).
        - **Red**: No match found.

14. **Extract Reconciled Data**: 
    - Once reconciliation is complete, new columns are added automatically (e.g., `----: ID`, `----: Type`, `----: Score`).
    - Use **Edit column > Add column from reconciled values...** to extract specific details like the **Biblissima URI** or **Preferred Label**.
