...
_Irises_ by Vincent van Gogh (Getty Museum) — a full IIIF manifest in Mirador, with multiple canvases, metadata, and annotations:
...
- Data is **siloed** — not designed to link to the Rijksmuseum's database
...
- **Resolvable**: points to current location via standard resolver
...
::: {.callout-note collapse="true" title="More context"}
DOI is the industry standard for academic publishing, ORCID for researchers, ROR for institutions, and ARK for cultural heritage objects. Each PID type has a dedicated resolver: doi.org, orcid.org, ror.org, hdl.handle.net, and n2t.net (ARK).
:::
...
- **Open Access (OA)**: free online access to scholarly publications — Gold, Green, Diamond, Bronze…
...
- **Open Government Data (OGD)**: public-sector data published proactively — replaces closed-by-default custody and fixed retention periods
...
- **Linked Data & Triplestore**: global URIs as identifiers; triples (subject → predicate → object) as the data model; SPARQL as the query language
- **PIDs**: unique, persistent, resolvable — the same entity recognised across every institution

:::

::: {.callout-note collapse="true" title="More context"}
**Open Science Landscape**: OA covers scholarly publications; ORD extends the logic to any dataset produced by research — the two are related but distinct, and funders increasingly mandate both. OGD applies open data principles to the public sector: for archives, this overturns the traditional logic of retention periods and scheduled releases in favour of proactive, early publication.
...
**Technical Stack**: PIDs and Linked Data together realise the Web of Data — the vision of the same entity (`wd:Q5582`, Van Gogh) linked across the Getty, Wikidata, and the Rijksmuseum, navigated seamlessly across institutional boundaries.
:::
